...
_IgnorarAbajo
 ...
 if joy0fire && d=0 && p=1 then c=1 : ballx=x : bally=y-5 : sounda = 5
 if joy0fire && d=1 && p=1 then c=2 : ballx=x+9 : bally=y-5 : sounda = 5
 
 if sounda > 0 then sounda = sounda - 1 : AUDC0 = 4 : AUDV0 = 4 : AUDF0 = sounda else AUDV0 = 0
 
 if c=1 then ballx = ballx-1
 if c=2 then ballx = ballx+1
 
 if ballx<0 then c=0
 if ballx>160 then c=0
 
 if c=0 then ballx=0 : bally=0
 
 if i=1 then player3x=player3x-1
 if i=1 && player3x=0 then player3x=160 : player3y = (rand&30)+20
 
 ;if collision(missile0,player3) then c=0 : i=0 : soundb=25 : score = score + 10
 if ballx>player3x-7 && ballx<player3x+1 && bally>player3y-7 && bally<player3y+1 then i=0 : score=score+10 : soundb = 50
 if i=0 then player3x=0 : player3y=0
 
 ;moving Enemy 2
 if j=1 then player2x=player2x+1
 if j=1 && player2x>160 then player2x=0 : player2y = (rand&30)+20 ;( 20 - 50 )
 
 if ballx>player2x-7 && ballx<player2x+1 && bally>player2y-7 && bally<player2y+1 then j=0 : score=score+10 : soundb = 50
 ...
 if y<9 then goto _ReiniciarNivel;y=9
 ;if y>88 then goto _ReiniciarNivel
